# 🛠️ tunescript — Implementation Guide
...
${JSON.stringify(lines)}`;
  ...
  if (!res.ok) throw new Error(`[Tunescript] Gemini API ${res.status}: ${await res.text()}`);
...
async function handleSongChange({ title, artist, duration, platform, getDomLyrics }) {
  ...
  if (!settings.geminiApiKey) {
    showErrorPanel('Add your Gemini API key in the tunescript extension settings.');
    return;
  }
  ...
  try {
    ...
    console.error('[Tunescript] Gemini error:', e);
    return;
  }
...
function showLoadingPanel() {
  ...
  panel.innerHTML = `
    <div class="tunescript-loading">
      <div class="tunescript-skeleton"></div>
      <div class="tunescript-skeleton short"></div>
      <div class="tunescript-skeleton"></div>
      <div class="tunescript-skeleton short"></div>
      <div class="tunescript-skeleton"></div>
    </div>
...
function hideLoadingPanel() {
  ...
  if (panel) panel.querySelector('.tunescript-loading')?.remove();
}

function showErrorPanel(msg) {
  const panel = document.getElementById('tunescript-panel') || createPanel();
  panel.innerHTML = `<div class="tunescript-error">${msg}</div>`;
}
...
#tunescript-panel::-webkit-scrollbar { display: none; }
...
.tunescript-line.tunescript-active {
  opacity: 1;
  transform: scale(1.02);
}

.tunescript-line.tunescript-past  { opacity: 0.22; }
.tunescript-line.tunescript-future { opacity: 0.38; }

.tunescript-romanized {
  font-size: 1.15em;
  font-weight: 600;
  color: #fff;
  margin: 0 0 5px 0;
...
/* Loading skeleton */
.tunescript-loading { padding: 8px 0; }

.tunescript-skeleton {
  height: 16px;
  background: linear-gradient(90deg, rgba(255,255,255,0.08) 25%, rgba(255,255,255,0.15) 50%, rgba(255,255,255,0.08) 75%);
  background-size: 200% 100%;
  animation: tunescript-shimmer 1.5s infinite;
  border-radius: 8px;
  margin-bottom: 20px;
  width: 80%;
}

.tunescript-skeleton.short { width: 50%; }

@keyframes tunescript-shimmer {
...
# 4. Select the tunescript/ folder
...
3. Open the tunescript extension popup → paste key into API Key field
...
```
I'm building a personal Chrome extension called tunescript that romanizes and 
