# 🎵 tunescript — Product Requirements Document

> **Project Name:** tunescript (अक्षर) — Sanskrit/Hindi for "letter" or "character"; also means "imperishable" — fitting for lyrics that transcend scripts  
...
Most Indian language lyrics (Hindi, Tamil, Malayalam, Kannada, Telugu) appear in their native scripts on streaming platforms. A growing segment of listeners — including Gen Z users raised with Roman-script digital environments — can understand these languages aurally but cannot read the native script fluently. Apple Music solves this with a native "Romanization" pronunciation feature. tunescript brings that capability to Spotify Web and YouTube Music.
